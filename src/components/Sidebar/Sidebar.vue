<template>
  ...
  <!-- 侧边栏容器 -->
  <div class="sidebar-wrapper" :class="{ 'sidebar-closed': !sidebarOpen, 'sidebar-mobile': isMobile }">
    <aside class="sidebar">
      <!-- 用户卡片 -->
...
onMounted(async () => {
  ...
  try {
    const [bars, posts] = await Promise.all([
      barsApi.getAll(),
      postsApi.getAll()
    ])
    allBars.value = bars || []
...
const hotBars = computed(() => {
  return [...allBars.value]
      .sort((a, b) => (b.member_count || b.memberCount || 0) - (a.member_count || a.memberCount || 0))
      .slice(0, 5)
})
...
.sidebar-wrapper.sidebar-closed .sidebar {
  width: 64px;
  padding: 20px 8px;
}
...
.sidebar-wrapper.sidebar-closed .collapse-btn {
  margin-left: 0;
